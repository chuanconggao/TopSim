...
def findBest(
    ...
    h: List[float] = []
    m: Dict[float, List[int]] = defaultdict(list)
    ...
    for (i, item) in enumerate(rStr):
        ...
        for (ln, p) in sIndex[item]:
            ...
            if totalNum > k:
                currWorstSim = h[0]
                currWorstNum = len(m[currWorstSim])
            ...
            if not tie and totalNum > k:
                del m[worstSim][(len(m[worstSim]) - (totalNum - k)):]
                totalNum = k


    return sorted(m.items(), key=lambda x: x[0], reverse=True)
